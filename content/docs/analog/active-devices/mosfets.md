...
For a MOSFET in saturation: g_m = 2 × I_D / (V_GS - V_th)
...
- Not the same as "BJT saturation" — this catches people. MOSFET saturation is where amplification happens. BJT saturation is where the device is fully on as a switch
...
- **Source degeneration** — A source resistor stabilizes the operating point. V_GS = V_G - I_D × R_S. If I_D increases, V_GS decreases, limiting the increase. Similar philosophy to BJT emitter degeneration
...
The same physical device serves both purposes — the difference is entirely in how it's biased and which region it operates in.
...
- For power switching, select MOSFETs by R_DS(on) and ensure V_GS is well above V_th for full enhancement
- For analog amplification, bias in saturation and calculate g_m from the operating current
- Use logic-level MOSFETs when driving from 3.3 V or 5 V logic without a gate driver
...
- A power MOSFET that runs hot in a switching application may not be fully enhanced — verify V_GS is well above V_th
- Unexpected oscillation in a MOSFET circuit often traces to Miller capacitance and high gate drive impedance — add a gate resistor or use a lower-impedance driver
