...
Before designing any interface circuit, characterize both sides of the connection. Skipping this step is how MCU pins die.
...
- [ ] **What current does the signal source deliver?** A 4–20 mA loop is a current source. An RS-232 driver can source/sink 10–20 mA. A relay contact closure delivers whatever the pull-up circuit provides. Your interface must not load the source beyond its capability
...
- [ ] **Load impedance?** The input impedance of your receiving circuit determines how much it loads the source. A CMOS microcontroller input is >10 Mohm at DC. An ADC input may have a sample-and-hold capacitor that draws transient current. A legacy audio input may be 10 kohm or 600 ohm depending on era and design
...
- [ ] **Where is ground?** Legacy equipment may reference to chassis (connected to mains earth), to a signal ground bus (which may or may not be connected to earth), or to nothing identifiable (floating). Measure the voltage between the signal ground of the legacy system and the ground of your modern system before connecting them. If there's more than a volt or two, you need isolation
...
- [ ] **Safety isolation?** If the legacy system operates at mains voltage or connects to mains-referenced circuits, you need safety-rated isolation (reinforced insulation, creepage/clearance, certified components). This is non-negotiable
...
- [ ] **Fault protection?** If miswiring, a component failure, or a surge could put dangerous voltage on the interface, isolation protects your modern equipment from destruction and protects you from shock
...
- [ ] **What happens if the cable is connected while both systems are powered?** Hot-plugging creates transient currents through connector capacitance and can inject spikes through ESD and charge redistribution. If the connector doesn't have ground-first/signal-last pin sequencing, every hot-plug is an ESD event
- [ ] **What happens if one system is powered and the other isn't?** If your MCU is off and the legacy system drives a signal into an MCU pin, the signal may backfeed through the MCU's ESD clamp diodes into the unpowered VCC rail, potentially powering part of the MCU through an unintended path (parasitic power-up). This causes latch-up and damage
- [ ] **What happens during a fault on the legacy side?** A shorted winding in a transformer, a stuck relay, a failed voltage regulator — these can put unexpected voltages on the interface. How much voltage can your interface circuit survive without passing damage through to the MCU?

## Step 2: What Can Kill Your MCU?

Modern CMOS microcontrollers are fragile compared to the legacy systems they're connecting to. Here's what destroys I/O pins and what creates latent damage.
...
**How it happens at the interface:** An RS-232 signal at +12V drives directly into a 3.3V MCU pin. A 24V industrial signal connected through a voltage divider where someone forgot the divider and connected directly. A legacy system with a pull-up to 5V connected to a 3.3V MCU that isn't 5V-tolerant.
...
If the two systems don't share a common ground, or if the shared ground has significant impedance, the "ground" potential of the legacy system may differ from the MCU's ground by hundreds of millivolts to volts. Every signal from the legacy system appears shifted by this offset when measured by the MCU.

**What it damages:** If the offset is large enough that legacy signals go below the MCU's VSS, you get negative voltage on the input pins (see above). Even if it doesn't cause direct damage, the offset shifts all signal levels and can push nominally-valid signals outside the MCU's input thresholds.
...
When the MCU is unpowered but an external signal is applied to an I/O pin, current flows through the pin's ESD clamp diode into the VDD rail. This partially powers the MCU through an unintended path — some internal circuits power up, others don't, and the resulting state can cause latch-up or permanent damage.
...
**Prevention:** Add series resistors (1–10 kohm) on I/O pins to limit backfeed current below the clamp diode's rating. Or use interface circuits with their own supply enable that tri-states outputs when the MCU is unpowered.

## Step 3: Common Adapter Building Blocks

These are the standard circuit patterns for bridging domains. Mix and match based on your domain identification checklist results.
...
When the source impedance and load impedance are mismatched, or when the source can't drive the load without distortion:
...
Use when the domain identification checklist reveals safety risk, ground loops, or ground offsets that exceed the MCU's tolerance.
...
- **TVS diodes:** Clamp fast transients (ESD, inductive kick) that the series resistor can't absorb alone. Bidirectional TVS for signals that swing both ways; unidirectional for DC-only. Place between the signal line and ground, after the series resistor, as close to the connector as possible
...
Ground strategy is the most commonly botched aspect of legacy interfacing. Get this wrong and everything else is noise.

### Single-Point Grounding

Connect the grounds of the legacy and modern systems at exactly one point. All signal returns flow through that single connection. This prevents ground loops by ensuring there's only one path for ground current.

**When it works:** Low frequencies (audio, slow digital), short cable runs, systems where you can control all ground connections.
...
- Bond your modern system's ground to the legacy chassis ground at the interface point
- Keep the bond short and low-impedance (wide braid or strap, not a long wire)
- Don't create additional ground connections elsewhere — one point of contact between the two ground systems
- If the chassis bond creates unacceptable noise, insert isolation at the signal level instead of trying to lift the ground
...
**Watch for:** Null modem vs. straight-through wiring. If the legacy device is DTE (like a PC), it expects DCE pinout on the cable — TX on pin 3, RX on pin 2. If both devices are DTE, you need a null modem crossover (TX to RX, RX to TX). Getting this wrong doesn't cause damage but nothing communicates.
...
**Watch for:** A broken wire in a 4–20 mA loop reads 0 mA. The 4 mA "zero" is specifically chosen so that a broken wire (0 mA) is distinguishable from a minimum reading (4 mA). If your firmware treats 0 mA as a valid reading instead of a fault, you'll miss broken sensors.
...
- For MOSFET drive: a logic-level MOSFET (IRLML2502 or similar) doesn't need the base resistor. Gate direct from GPIO (or through a small series resistor for EMI), source to ground, drain to relay coil
...
- The coupling capacitor blocks DC. For audio-frequency response down to 20 Hz into a 10 kohm load, the cap needs to be at least 1/(2π × 20 × 10000) ≈ 0.8 µF — so 1–10 µF is typical
...
**Watch for:** Audio grounds between equipment. Connecting the MCU ground to the audio equipment ground may create a ground loop with audible hum. If it does, insert an audio isolation transformer (Triad Magnetics TY-250P or similar 600:600 ohm) in the signal path.

## Gotchas

- **"It works on the bench" doesn't mean it works in the field.** Bench conditions are clean: short cables, common ground through the bench supply, no nearby motors or relays. In the field, cables are long, grounds differ, and EMI is real. Test with realistic cable lengths and with both systems powered from their intended sources
- **Series resistors on every interface pin are the cheapest insurance.** Even 100 ohms limits fault current and reduces EMI coupling. The performance cost is negligible for signals under 1 MHz. Make this a default habit, not an afterthought
- **Isolation doesn't help if the isolated supply shares the same input power.** If both sides of an optocoupler are powered from the same wall adapter through a non-isolated DC-DC module, there's no real isolation — the ground paths connect through the shared supply. True isolation requires separate power domains or a certified isolated DC-DC converter
- **Legacy systems assume their loads are passive.** A legacy 4–20 mA transmitter expects to drive a passive 250 ohm load, not an active circuit that might inject current back into the loop. A tube-era audio output expects a resistive load, not a capacitive one that reflects energy. Respect the legacy system's assumptions about what it's driving
...
- **Ground voltage between systems can drift with load.** You measure 0V between the grounds with a DMM, connect the interface, and now there's 500 mV of offset because the interface cable carries return current through a high-impedance path. Measure the ground offset with the interface active and current flowing, not just at idle
- **USB ground is not isolated.** USB-connected devices share ground with the host PC. If the legacy system has a ground offset relative to the PC, that offset appears across the interface. A USB isolator module (Adafruit has a cheap one based on the ADUM4160) breaks this connection
...
- [ ] Measure continuity on each signal path from the legacy connector through your interface circuit to the MCU pin. Confirm no shorts between adjacent pins
...
- [ ] Start with read-only operations. If you're reading a sensor, read it and verify the value makes sense. If you're reading a contact closure, toggle the contact and watch the GPIO
