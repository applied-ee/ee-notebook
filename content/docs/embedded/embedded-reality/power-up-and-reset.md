...
- **A subsystem that behaves differently after a reset versus after a power cycle** is showing, through the control lens, that the reset doesn't restore all control state to the power-on default — some configuration, calibration, or mode state persists through the reset and causes the subsystem to start from a different state than the power-on state.
