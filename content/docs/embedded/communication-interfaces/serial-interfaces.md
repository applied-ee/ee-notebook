...
For all four buses, the firmware must decide how data moves between the peripheral and memory. The options are polling, interrupts, and DMA — though for I2S, DMA is effectively the only viable choice for continuous streaming.
...
**DMA** moves data without CPU involvement at all. Essential for high-throughput SPI transfers (reading a flash chip, driving a display), continuous I2S audio streaming, and high-rate ADC acquisition. See [DMA]({{< relref "dma" >}}) for the details.
...
**I2S** — audio streaming: codecs, DACs, ADCs, digital microphones. Dedicated to moving audio samples at fixed rates. Not a general-purpose data bus — it carries audio, and it does it well.

If the peripheral offers I2C and SPI, and you have the pins, SPI is almost always easier to bring up and debug. I2C's strength is pin efficiency, not ease of use. I2S is not interchangeable with the others — it exists specifically for audio, where continuous fixed-rate streaming and precise clocking matter more than flexibility.
