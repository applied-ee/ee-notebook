...
- **An ADC reading that has a noise floor higher than the datasheet specification, with noise concentrated at specific frequencies,** often indicates internal substrate coupling from a digital peripheral (timer, PWM, communication interface) operating at those frequencies — the noise source is inside the IC, coupled through the substrate rather than through external paths.
