...
L_leakage ≈ L_winding x (1 - k)

For a transformer with a 1 mH primary and k = 0.99, the primary leakage inductance is about 10 uH. That sounds small, but at fast switching speeds it's enough to cause serious problems.

### Why It Matters

Leakage inductance stores energy (1/2 x L_leak x I^2) that can't transfer to the secondary through the core. When current through the winding is interrupted — as it is every switching cycle in a switching power supply — this stored energy has to go somewhere. If there's no designed current path, the voltage across the switch rises until something breaks down:

V_spike = L_leak x dI/dt

A 10 uH leakage inductance with current dropping from 2A to 0A in 50 ns produces a 400V spike. This is why every switching converter with a transformer needs a snubber or clamp circuit — the spike is a direct consequence of leakage inductance.
...
## Practical Impact by Application

### Switching Power Supplies

Leakage inductance is the primary parasitic concern. It stores energy that must be managed every switching cycle. The snubber/clamp choice directly affects efficiency, EMI, and switch stress. Inter-winding capacitance determines common-mode noise performance and EMI filter requirements.

### Audio Transformers

Leakage inductance rolls off high frequencies (it's a series inductance in the signal path). Stray capacitance creates a resonance peak and then rolls off further. Together, they define the usable bandwidth of the transformer. High-quality audio transformers minimize both through interleaved windings and careful geometry.

### Gate Drive Transformers / Pulse Transformers

Leakage inductance slows down edges and adds delay. Stray capacitance distorts narrow pulses by charging and discharging on each edge. For gate drive applications where fast, clean edges are critical, low-leakage, low-capacitance construction is essential.
...
- **A snubber that "works" may be wasting significant power.** RC snubbers are easy to add but burn power proportional to C × V² × f. On a converter switching at 200 kHz with a 100 pF snubber cap and 100V swing, that's 0.2W per snubber — potentially a significant fraction of total losses in a low-power converter. Size the snubber for the minimum capacitance that damps the ring adequately
...
- **Self-resonant frequency applies to inductors too.** A filter inductor that's a perfect inductor at 100 kHz may be capacitive at 10 MHz. If you're counting on it for EMI filtering at high frequencies, check the SRF. Above it, the inductor is actually making things worse
