...
The voltage divider equation assumes no current flows out of the output node (or equivalently, the load impedance is infinite). The moment a load is connected, the output voltage changes.

A load resistance R_load in parallel with R2 replaces R2 with the parallel combination:

**V_out_loaded = V_in × (R2 || R_load) / (R1 + R2 || R_load)**
...
The load should be at least 10× the divider's output impedance (R1 || R2, or approximately R2 if R2 << R1 isn't true, the Thévenin output resistance is R1 || R2) for the loading error to be under about 10%.

For less than 1% error, the load should be at least 100× the source impedance.

## Output Impedance

The voltage divider has an output impedance equal to R1 in parallel with R2:

**Z_out = R1 || R2 = (R1 × R2) / (R1 + R2)**

This is the Thévenin equivalent output resistance. It determines:

- How much the output voltage drops when loaded
- The time constant when driving a capacitive load (τ = Z_out × C_load)
...
- Time constant: τ = Z_out × C_load
- Bandwidth: f_3dB = 1 / (2π × Z_out × C_load)

A 10 kΩ divider driving a 100 pF load (typical ADC input) has a time constant of 1 µs and a bandwidth of about 160 kHz. For slowly-changing signals, this is fine. For fast signals, a lower impedance divider or a buffer is needed.
...
- **Capacitive divider:** V_out = V_in × C1 / (C1 + C2) — note the capacitances are "inverted" compared to resistors because Z_C = 1/(jωC). The smaller cap gets more voltage
...
- Match divider impedance to the load: Z_out should be at least 10× lower than Z_load
- For precision dividers, use 1% or better tolerance resistors
- Buffer high-impedance dividers with an op-amp voltage follower if loading is a concern

## Caveats

- The unloaded voltage divider equation is almost never the whole story — Any real circuit connected to the output is a load. Even a 10 MΩ oscilloscope probe loads a high-impedance divider
- Divider impedance and noise — Higher impedance = more thermal noise (V_noise = √(4kTRB)). In precision or low-noise circuits, this sets a practical lower limit on bandwidth or an upper limit on impedance
...
- Slow response or rounding of fast edges on a divider output indicates capacitive loading and the RC time constant
- Unexpected noise on a divider output may indicate the impedance is too high for the environment
