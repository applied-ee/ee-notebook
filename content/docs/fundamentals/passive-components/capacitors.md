...
- **ESL (Equivalent Series Inductance)** — Lead and body inductance. At high frequencies, a capacitor becomes inductive. The self-resonant frequency (SRF) is where capacitance and inductance cancel, and above it the "capacitor" behaves as an inductor (see [Frequency-Dependent Behavior]({{< relref "../ac-impedance/frequency-dependent-behavior" >}}) for impedance curves and SRF details)
