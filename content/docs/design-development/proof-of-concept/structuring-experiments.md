...
The pass/fail criteria come from the [requirements]({{< relref "../ideation-and-requirements" >}}). If the requirements say "detect vibration below 10 Hz," the POC must test at frequencies below 10 Hz with a known reference vibration. Testing at 100 Hz and assuming the sensor will also work at 10 Hz is not a valid POC.
...
- **Do not optimize the POC.** The goal is to answer a question, not to build the best possible breadboard — if hours are being spent tweaking component values on a breadboard to squeeze out another dB of performance, stop; the question has been answered ("it is marginal") and the optimization belongs in the [schematic design]({{< relref "../schematic-design" >}}) phase
