...
- **Copper-to-edge distance.** If copper needs to extend to the edge (for edge connectors, for instance), this requires special handling — typically a plated edge or castellated vias, both of which need to be communicated to the fab.

These constraints vary between manufacturers and between their process tiers. A board designed for one fab's premium process may violate another fab's standard rules. Always download and import the specific fab's design rules into the EDA tool before starting layout.

## Assembly DFM

Fabrication gets a bare board. Assembly gets components onto it. Assembly DFM is about ensuring that components can be placed accurately and soldered reliably, whether by hand, by pick-and-place machine, or by a combination of both.
...
- **Fiducials.** Pick-and-place machines need reference marks to align the board. Global fiducials (two or three per board) establish overall alignment. Local fiducials near fine-pitch components (QFP, BGA) provide additional accuracy where it's needed most. For builds of more than a handful of boards, adding fiducials is worthwhile — they're free copper dots that save enormous assembly headaches.
...
- **Components too close for rework.** Placing a 0402 capacitor right next to a QFP lead means neither one can be reworked without disturbing the other. Design for the possibility of rework, not just initial assembly.
...
Most PCB fabrication houses offer a free DFM review. After design files are uploaded, their engineering team checks for violations against their specific process capabilities and sends back a report highlighting problems. Use this service every time. It costs nothing and catches issues that the EDA tool's DRC might miss because the design rules don't perfectly match the fab's actual capabilities.
...
Memorizing IPC standards isn't necessary, but knowing they exist and where to look is valuable:
...
Even for hobby work, understanding the IPC-A-610 criteria for acceptable solder joints improves the ability to inspect one's own work.

## Panelization

For builds of more than a few boards, panelization — arranging multiple copies of the board on a single larger panel — reduces cost and simplifies handling. The panel moves through the fab and assembly process as one piece, then individual boards are separated afterward.
...
The fab house can usually design the panel if the method is specified and board dimensions are provided, but understanding the options helps in designing boards with appropriate edge features.

## Clean Design Files

The manufacturing data package is the complete set of files needed to build and assemble a board. An incomplete package causes delays, questions, and errors. At minimum:
...
Generate these files fresh from the final design — don't reuse files from an earlier revision. Every file in the package must correspond to the same design state. A mismatch between the gerbers and the BOM is one of the most common causes of assembly errors, and it's entirely preventable.

## Tips

- Import the fab house's specific design rules into the EDA tool before starting layout, rather than relying on default trace widths and clearances
- Always submit designs for the fab's free DFM review — it catches issues that the EDA tool's DRC misses and costs nothing but a day of lead time
- Add fiducials and test points early in layout while board space is available; retrofitting them later often forces compromises
- Generate all manufacturing files fresh from the final design revision and verify they all correspond to the same design state before sending

## Caveats

- **The EDA tool's defaults aren't DFM-safe.** Default trace widths, via sizes, and clearances are often set for ease of routing, not manufacturing robustness. Import the fab's design rules and use them from the start
- **DFM review is not the same as DRC.** Design Rule Check in the EDA tool verifies internal rules. DFM review at the fab verifies their process. Both are necessary, and they catch different things
- **Hand assembly has different DFM constraints.** For hand-soldering, the considerations shift: iron access, visible joints, and components large enough to handle are needed. A design optimized for machine assembly can be miserable to hand-build
