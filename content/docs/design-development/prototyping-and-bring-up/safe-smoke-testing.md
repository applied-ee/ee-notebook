...
A current-limited bench power supply is the single most important tool for first power-on. Unlike a wall adapter or battery, a bench supply allows setting a maximum current that the supply will deliver. If the board tries to draw more than this limit, the supply drops its voltage to maintain the current ceiling — the board gets less voltage, but it doesn't get unlimited current to burn through traces and destroy components.
...
- If the expected current is unknown, start with a low limit (50-100 mA) and increase gradually.

**What the current reading indicates:**
...
For boards with uncertain behavior, or for high-power designs where a fault could be destructive:
...
This approach is especially valuable for designs with multiple voltage regulators that cascade (a 12V input feeds a 5V regulator, which feeds a 3.3V regulator). As the input ramps up, each regulator comes online in sequence, and each one's startup can be observed.
...
- **Oscillation.** Some regulators oscillate when their output capacitor is wrong or missing. The voltage and current will fluctuate rapidly, appearing on the bench supply as a flickering voltage or current reading. Check with an oscilloscope for confirmation.
...
- **Load switches with enable control.** Power distribution through load switches or enable-controlled regulators allows bringing up sections of the board independently. This isolates faults to specific subsystems.
...
3. **Analyze why.** Was the component reversed? Was it overloaded? Did a design error put the wrong voltage on it? Is the footprint wrong, connecting the wrong pins? Understanding the cause is more important than replacing the component — if the failure isn't understood, the replacement will fail too.
...
Do not apply power again until the failure is understood. "Maybe it was just a bad component" is rarely the correct explanation.
...
A simple version of this test is to leave the board running on the bench while doing other work, glancing at it occasionally. A more rigorous version uses automated monitoring: log supply current, board temperature, and key output parameters over time.

## Tips

- Always use a current-limited bench supply for first power-on — set the limit to 1.5-2x the expected idle current and ramp the voltage up gradually
- Watch the current reading at each voltage step during ramp-up; a sudden jump or failure to settle reveals problems before they become destructive
- Design boards with protection features (polarity protection, fuses, load switches) from the start to make smoke testing safer and fault isolation easier
- After initial bring-up succeeds, run an extended thermal soak test (8-24 hours) to catch marginal components that pass short-duration testing

## Caveats

- **Current limit set too low prevents startup.** Switching regulators and MCUs with large decoupling capacitance need a brief inrush current that may exceed the idle current by 5-10x — if the current limit is too aggressive, the board will never start
...
- **Not all failures produce smoke.** A reversed zener diode might quietly clamp a signal to the wrong voltage, or a misplaced feedback resistor might set a regulator to 4.2V instead of 3.3V; silent failures are harder to find than dramatic ones
