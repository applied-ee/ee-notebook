...
**Voltage.** Use a multimeter for the DC value and an oscilloscope for the full picture. A multimeter shows the average voltage; a scope shows the peak-to-peak ripple and any transients.
...
- Linear regulators should show very low ripple. If significant noise appears on a linear regulator's output, check the input bypassing and ensure the regulator isn't oscillating (some LDOs oscillate with certain output capacitor ESR values).
...
If the design includes current sense resistors (small-value resistors in series with each power rail), individual rail currents can be measured without breaking the circuit. Measure the voltage across the sense resistor and calculate I = V / R.

If sense resistors aren't present (common on first prototypes), several alternatives exist:
- **Measure at the power supply.** A bench supply with a current readout gives the total board current.
...
A subtle but important test: does the circuit's behavior change when the supply voltage varies?
...
- Always verify power rails with an oscilloscope in addition to a multimeter — a multimeter hides ripple and transient issues that can cause subtle downstream failures
- Measure each supply rail under realistic load conditions, not just at idle — regulators that look fine unloaded may droop or oscillate when the full circuit is active
- Capture startup waveforms on all power rails simultaneously to verify sequencing before enabling any active circuitry
- Record measured current draw for each rail as a baseline; deviations in future revisions immediately flag regressions or unintended changes

## Caveats

- **Multimeter readings hide ripple.** A multimeter shows the average DC voltage — a supply rail with 3.3V average and 500 mV peak-to-peak ripple reads "3.3V" on the meter but is far from a clean 3.3V
...
- **Current limiting can mask startup problems.** If the bench supply's current limit is set too low, a regulator may not start up properly — it needs a brief inrush current to charge the output capacitors
- **Sequencing violations don't always cause immediate damage.** A device that survives incorrect power sequencing today may develop latent damage that causes failure later — a working board doesn't guarantee correct sequencing
