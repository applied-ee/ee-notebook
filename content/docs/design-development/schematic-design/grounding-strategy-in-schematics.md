...
Grounding strategy is a schematic-level decision. If grounding is deferred until layout, the design has already committed to a topology that may not support clean separation. The schematic is the place to define ground domains, decide how they connect, and communicate that intent to whoever does the layout — including a future version of the same designer.
...
None of these show up in a schematic simulator, because simulators treat GND as a perfect node. They must be designed around deliberately.

## Ground Domains

A ground domain is a region of the circuit that shares a common return path. In a typical mixed-signal design, there might be several:
...
Most real designs use a hybrid: multi-point grounding within each domain (short vias to a local ground plane), with single-point connections between domains. This provides low inductance within each domain and current isolation between domains.
...
**One GND symbol for everything.** The schematic shows a single GND net, and the designer assumes the layout person will "figure out" the separation. They won't — or they'll do it differently than intended.

**Connecting grounds at multiple points.** If AGND and DGND are separate nets, they should connect at exactly one point. Multiple connections defeat the purpose of separation by creating ground loops between the domains. If multiple connections are needed, one domain is probably more appropriate than two.
...
**Forgetting the current path.** Ground is a return path, not a dumping ground. Every milliamp that flows out of a supply pin flows back through ground. Trace the current mentally: where does the motor driver's 2 A return current actually flow? If it shares a path with the ADC's reference ground, that's a problem that no amount of layout optimization will fix.
...
- **Annotate ground pins.** On ICs with both analog and digital ground pins (common on ADCs, DACs, and mixed-signal processors), label which net each pin connects to. Some ICs have internal connections between their ground pins — the datasheet will specify this, and the schematic should reflect the chosen configuration.
...
When in doubt, start with one ground and a solid ground plane. Add domain separation only when there is a specific noise coupling problem that separation solves. Unnecessary ground splits create more problems than they prevent — especially if a signal trace crosses the split boundary.

## Tips

- Use distinct net names (`AGND`, `DGND`, `PGND`) in the schematic rather than a single `GND` symbol with layout notes -- explicit naming prevents misinterpretation
- Draw the ground domain connection point on the schematic as a real component (zero-ohm resistor, ferrite bead, or wire) with an annotation explaining the choice
- Add a simple block diagram on the first schematic sheet showing ground domains and their connection topology to give immediate context to anyone reading the design
- Start with a single solid ground plane and only split into domains when a specific, identified noise coupling problem demands it

## Caveats

- **A single GND symbol hides design intent** — relying on a layout note to communicate ground separation almost guarantees it will be done differently than intended or missed entirely
- **Unnecessary ground splits create more problems than they solve** — a signal trace crossing a ground plane split loses its return path, creating a slot antenna that radiates; split only when a specific coupling problem demands it
- **Star grounding fails at high frequencies** — long ground runs to a single point add inductance that degrades performance above a few MHz; star topology is for audio and DC precision, not for fast digital or RF
- **Multiple connections between separated ground domains defeat the separation** — if AGND and DGND connect at more than one point, a ground loop forms and return current flows through both paths unpredictably
- **Ground is a return path, not a dumping ground** — every milliamp flowing out of a supply pin returns through ground; failing to trace where high-current return paths actually flow leads to noise coupling that no amount of layout optimization can fix
