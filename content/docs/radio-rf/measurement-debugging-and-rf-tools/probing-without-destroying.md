...
For a tuned circuit like a bandpass filter or oscillator tank, even 1-2 pF of added capacitance shifts the resonant frequency. A filter tuned to 100 MHz with a total tank capacitance of 10 pF sees a 10% shift (down to about 95 MHz) when 1 pF is added with a probe. That shift can move the passband enough to completely change the circuit's behavior — making the measurement meaningless because the circuit is no longer operating as it normally would.
...
- **Magnetic field probes (H-field)**: Small loops of wire that couple to the magnetic field component. They are sensitive to current flow — useful for finding current paths, identifying which traces carry the most RF current, and locating coupling between traces. A small loop (5-10 mm diameter) provides spatial resolution on the order of its diameter.

- **Electric field probes (E-field)**: Short monopoles or open-ended coaxial structures that couple to the electric field. They are sensitive to voltage — useful for finding high-voltage nodes, identifying radiating points, and mapping electric field patterns.
...
The best approach to RF measurement is to design test access into the circuit from the beginning, so probing during debugging is never necessary.
...
- **SMA or U.FL connectors**: Include RF connectors at key points — amplifier output, filter input/output, antenna feed. A $0.50 U.FL footprint on the PCB saves hours of debugging time when a cable can be connected directly to a spectrum analyzer or VNA without probing.

- **Test points with guard traces**: If a probe-accessible pad must be used, surround it with grounded guard traces to provide a nearby ground connection and reduce stray coupling.
...
3. **Minimize ground lead length**: If a probe must be used, remove the ground clip and use the shortest possible ground connection.
4. **Verify that probing does not change behavior**: Measure a known parameter (DC bias, frequency) with and without the probe. If the value shifts significantly, the probe is disturbing the circuit.
5. **Use the highest-impedance, lowest-capacitance probe available**: Active probes are always better than passive probes at RF.

## Tips

- Before probing any RF node, calculate the probe's capacitive impedance at the operating frequency — if it is within 10x of the circuit impedance, the measurement will significantly alter circuit behavior
- Include at least one SMA or U.FL test connector at each critical RF node during PCB layout; the cost is negligible compared to the debugging time saved
- When a near-field probe scan reveals a hot spot, try both H-field and E-field probes at that location — the H-field probe identifies current-carrying traces while the E-field probe identifies high-voltage nodes, together revealing the nature of the emission
- Keep a set of probe tip adapters (spring ground, solder-in ground) organized and accessible; switching from a ground clip to a spring ground takes seconds but eliminates the most common source of measurement error above 30 MHz

## Caveats

- **The circuit works until it is probed** — If connecting a probe stops an oscillator or detunes a filter, the probe is the problem, not the circuit. Reduce probe loading or use a non-contact method
- **Near-field probes are relative, not absolute** — A near-field probe can indicate that point A radiates more than point B, but not how many dBm either one produces. Use them for comparison and localization, not calibrated measurement
...
- A near-field H-field probe scan across a PCB shows strong signal pickup along a specific trace but not at the IC pin — this indicates the trace itself is radiating, likely due to poor impedance control or missing ground stitching
- Two probes on a differential pair cause a common-mode imbalance visible as increased even-order harmonics on the spectrum analyzer — removing one probe restores the expected spectral signature
