...
- **An analog measurement that's accurate at low signal levels but distorted at high signal levels** commonly appears when a component in the signal chain (a resistor, a capacitor, a transistor) is operating outside its linear range due to a bias point shift — the bias drift at the primitive level transforms into nonlinearity at the block level and inaccuracy at the subsystem level.
